...
--zmiana nazwy shcematu na nazwisko (schemat pusty)
ALTER SCHEMA schema_name RENAME TO Gadek;
...
CREATE TABLE Gadek.intersects AS
SELECT a.rast, b.municipality
FROM rasters.dem AS a, vectors.porto_parishes AS b
WHERE ST_Intersects(a.rast, b.geom) AND b.municipality ilike 'porto';

--Wykonać poniższe kroki, jeśli tworzy się tabele zawierające dane rastrowe

-- a) Dodanie serial primary key

ALTER TABLE Gadek.intersects
ADD COLUMN rid SERIAL PRIMARY KEY;

-- b) Utworzenie indeksu przestrzennego

CREATE INDEX idx_intersects_rast_gist ON Gadek.intersects
...
CREATE TABLE Gadek.union AS
SELECT ST_Union(ST_Clip(a.rast, b.geom, true))
FROM rasters.dem AS a, vectors.porto_parishes AS b
WHERE b.municipality ilike 'porto' and ST_Intersects(b.geom,a.rast);

-- ST_Union pozwala również na operacje na naładających się rastrach opartych na danej funkcji agregującej, czyli FIRST LAST SUM COUNT MEAN lub RANGE 
-- Na przykład
...
--rastrowanie tabeli z parafiami o takiej samej charakterystyce przestrzennej tj. wielkosci piksela, zakresy itp.

CREATE TABLE Gadek.porto_parishes AS
WITH r AS (
...
-- integer mogą również reprezentować liczby całkowite ujemne. 

-- Wynikowy rasterz tego zadania to jedna parafia na rekord, na wiersz tabeli. 
-- Trzeba użyć qGIS lub ArcGIS do wizualizacji wyników
...
DROP TABLE Gadek.porto_parishes; --> drop table porto_parishes first
CREATE TABLE Gadek.porto_parishes AS
WITH r AS (
			SELECT rast FROM rasters.dem
			LIMIT 1
...
CREATE TABLE Gadek.intersection as
...
CREATE TABLE Gadek.dumppolygons AS
...
CREATE TABLE Gadek.landsat_nir AS
SELECT rid, ST_Band(rast,4) AS rast
FROM rasters.landsat8;

-- Przykład 2 - ST_Clip - wycięcie rastra z innego rastra
--wycina jedną parafię z tabeli vectors.porto_parishes. 
CREATE TABLE Gadek.paranhos_dem AS
...
CREATE TABLE Gadek.paranhos_slope AS
SELECT a.rid,ST_Slope(a.rast,1,'32BF','PERCENTAGE') as rast
FROM Gadek.paranhos_dem AS a;

-- Przykład 4 - ST_Reclass - reklasyfikacja rastra

CREATE TABLE Gadek.paranhos_slope_reclass AS
SELECT a.rid,ST_Reclass(a.rast,1,']0-15]:1, (15-30]:2, (30-9999:3','32BF',0)
FROM Gadek.paranhos_slope AS a;

-- Przykład 5 - ST_SummaryStats - statystyki rastra
-- generacja statystyk dla kafelka
...
CREATE TABLE Gadek.tpi30 as
SELECT ST_TPI(a.rast,1) as rast
FROM rasters.dem a

-- a. Dodanie serial primary key

ALTER TABLE Gadek.tpi30
ADD COLUMN rid SERIAL PRIMARY KEY;

-- b. Utworzenie indeksu przestrzennego

CREATE INDEX idx_tpi30_rast_gist ON Gadek.tpi30
USING gist (ST_ConvexHull(rast));

-- c. Dodanie raster constraints

SELECT AddRasterConstraints('Gadek'::name,
...
CREATE TABLE Gadek.porto_ndvi AS
WITH r AS (
	SELECT a.rid,ST_Clip(a.rast, b.geom,true) AS rast
	FROM rasters.landsat8 AS a, vectors.porto_parishes AS b
	WHERE b.municipality ilike 'porto' and ST_Intersects(b.geom,a.rast)
...
ALTER TABLE Gadek.porto_ndvi
ADD COLUMN rid SERIAL PRIMARY KEY;

-- b) Utworzenie indeksu przestrzennego na wcześniej stworzonej tabeli

CREATE INDEX idx_porto_ndvi_rast_gist ON Gadek.porto_ndvi
USING gist (ST_ConvexHull(rast));
...
create or replace function Gadek.ndvi(
...
CREATE TABLE Gadek.porto_ndvi2 AS
WITH r AS (
	SELECT a.rid,ST_Clip(a.rast, b.geom,true) AS rast
	FROM rasters.landsat8 AS a, vectors.porto_parishes AS b
	WHERE b.municipality ilike 'porto' and ST_Intersects(b.geom,a.rast)
...
SELECT
	r.rid,ST_MapAlgebra(
		r.rast, ARRAY[1,4],
		'Gadek.ndvi(double precision[], integer[],text[])'::regprocedure, --> This is the function!
		'32BF'::text
	) AS rast
FROM r;
...
ALTER TABLE Gadek.porto_ndvi2
ADD COLUMN rid SERIAL PRIMARY KEY;

-- b. Utworzenie indeksu przestrzennego

CREATE INDEX idx_porto_ndvi2_rast_gist ON Gadek.porto_ndvi
USING gist (ST_ConvexHull(rast));
